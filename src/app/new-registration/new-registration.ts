...
export class NewRegistrationPage {
  ...
  async presentAlertSuccess(title: string, message: string, regNumber) {
    let alert = await this.alertCtrl.create({
      header: title,
      message: message + '<br><h1><center>' + regNumber + '</center></h1>',
      buttons: [
        {
          text: 'انهاء التسجيل',
          handler: () => {
  ...
  async loadClassess(p, b, g) {
    ...
    this.http.get(link)
      .subscribe(data => {
        ...
        if (p) {
          this.items = data[0].registration_classes[0].classes;
        } else if (g) {
          this.items = data[0].registration_classes[1].classes;
        } else if (b) {
          this.items = data[0].registration_classes[2].classes;
        }
      }, err => {
        alert(err);
      });
